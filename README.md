...
- **VIP 与随机交易区**：拥有 `primemc.perm.pro`/`primemc.perm.ultra` 权限的玩家可随地交易，普通玩家需前往自动生成的随机交易区。
- **NPC 管理系统**：内置商贩、市场经纪人等 NPC，支持通过命令创建、移动、改名、配置对话及行为。
- **原版工具强化**：启动时为主流工具批量应用名称、Lore、附魔和耐久倍率，保障泥土经济体验。
- **动态市场经济**：根据全服财富总量自动调整通胀倍率与店主抽成，店主可从交易额中抽取 20%~25% 佣金。
- **武器库与职业成长**：自定义武器与护甲可携带技能、粒子效果与额外属性，同时记录血量、魔法、防御、农业、战斗、钓鱼、射击等成长线。
...
| `/dirtshop stats` | 查看自身血量/魔法/防御等成长与各类经验 |
| `/dirtshop zones` | 查看随机交易区域、店主与当前通胀倍率 |
| `/dirtshop armory [list]` | 浏览武器库或（管理员）发放武器、护甲 |
...
- `economy`：设置服务器目标财富、商店通胀倍率以及店主抽成区间，自动影响商品价格与佣金。
- `trade-zones`：定义随机交易区生成世界、数量、半径范围以及可选的店主名字，普通玩家需在这些区域内交易。
- `stats`：设置血量、魔法、防御的基础值、每级成长以及魔法回复速度。
- `armory`：声明可发放的自定义武器和护甲，包含粒子特效、技能、魔法消耗与冷却时间。
...
- `player-stats.yml`：记录玩家血量、魔法、防御以及各类经验值。
